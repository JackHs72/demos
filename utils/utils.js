var utils = function () {
    ...
    */
    ...
    // 运动函数
    function move1(obj, json, callback) {
        ...
        obj.timer = setInterval(function () {
            ...
            for (var attr in json) {
                var targte = json[attr];
                // 判读属性是不是opacity
                if (attr === 'opacity') {
                    var speed = Math.round(getStyle(obj, attr) * 100); //100
                } else {
                    var speed = parseInt(getStyle(obj, attr)); // left
                }

                var dir = (targte - speed) / 9
                dir > 0 ? Math.ceil(dir) : Math.floor(dir);
                speed += dir;

                if ((speed > targte && dir > 0) || (speed < targte && dir < 0)) {
                    speed = targte;
                }
                ...
                if (speed !== targte) { // 条件满足，说明有当前值没有到达目标值
                    onOff = false; // 说明定时器不能停止
                }
                if (onOff) { //  
                    clearInterval(obj.timer);
                    callback && callback();
    ...
    */
    ...
    function hasClass(curEle, className) {
        ...
        return reg.test(curEle.className)[0];
    }
